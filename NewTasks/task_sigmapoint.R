...
#' @param cluster Set to false to not use cluster because it breaks
...
#' @export
runSigmaPoint <- function(alpha = 0.5, beta = 2, kappa = 3, var = NULL, experiments, mean_fit_as_basis = TRUE, cl = NULL, model = getCurrentModel(), cluster = FALSE, ...) {
  loadNamespace("dplyr")
  loadNamespace("tidyr")
  c_datamodel <- assert_datamodel(model)
  assert_that(
    is.null(alpha) || is.number(alpha),
    is.null(beta)  || is.number(beta),
    is.null(kappa) || is.number(kappa),
  ...
  param_vals_matrix <- as.matrix(param_vals)
    ...
    tryCatch({
    result_init <- CoRC::runParameterEstimation(experiments = data)
    results <- result_init$parameters[-(1:nrow(result_init$parameters)),] %>% add_column("run" = numeric())
    for (i in 1:length(data_list)){
      res     <- runParameterEstimation(experiment = data_list[[i]])
      res     <- res$parameters
      res     <- add_column(res, "run" = rep(i, nrow(res)))
      results <- add_row(results, res)
    }
    param_vals <-
      results %>%
      dplyr::select(.data$parameter, .data$value, .data$run) %>%
      tidyr::pivot_wider(names_from = .data$parameter, values_from = .data$value) %>%
      dplyr::bind_rows()
    param_vals_matrix <- as.matrix(param_vals[,-1])
    
    }, error = function(e){msg = "Something went wrong"
    stop()}
    )
  }
